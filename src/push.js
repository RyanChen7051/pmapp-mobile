...
export function setupPush(App) {
  ...
  const saveSubscription = async (sub) => {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`保存订阅失败 ${resp.status} ${txt.substring(0, 120)}`);
    }
  };

  // 用户手动启用（必须在用户手势中调用，iOS 才会弹权限框）
  App.enablePush = async function () {
    if (!App.pushSupported) { this.toast('当前浏览器不支持推送'); return false; }
    let perm = Notification.permission;
    if (perm === 'denied') { this.toast('通知权限已被系统拒绝，请到 iPhone 设置中开启'); return false; }
    if (perm === 'default') {
      try { perm = await Notification.requestPermission(); }
      catch { this.toast('请求通知权限失败'); return false; }
      if (perm !== 'granted') { this.toast('未授予通知权限'); return false; }
    }
    try {
  ...
  App.disablePush = async function () {
    try {
      ...
      this.toast('已关闭推送通知');
      this.renderPushSettings();
    } catch (e) { this.toast('关闭失败: ' + (e.message || e)); }
  };
  ...
  App.renderPushSettings = function () {
    ...
    if (!App.pushSupported) {
      panel.innerHTML = '<div class="setting-row"><span class="setting-label">推送通知</span><span class="setting-value" style="color:#94a3b8">此浏览器不支持</span></div>';
      return;
    }
    const perm = App.pushStatus();
    let statusText, btn;
    if (perm === 'denied') {
      statusText = '已拒绝（去系统设置开启）';
      btn = '<button class="btn btn-secondary" disabled style="opacity:.5">已拒绝</button>';
    } else if (perm === 'granted') {
      statusText = '已启用 ✅';
      btn = '<button class="btn btn-secondary" onclick="App.disablePush()">关闭推送</button>';
    } else {
      statusText = '未启用';
      btn = '<button class="btn btn-primary" onclick="App.enablePush()">启用推送</button>';
    }
    panel.innerHTML = `
      <div class="setting-row"><span class="setting-label">推送通知</span><span class="setting-value" id="push-status">${statusText}</span></div>
      <div style="padding:0 12px 8px">${btn}
        <div style="font-size:11px;color:#94a3b8;margin-top:6px">启用后，PMApp 的关键提醒会推送到 iPhone，并自动镜像到 Apple Watch。</div>
      </div>`;
  };
