...
export function setupMeetings(App) {
  ...
  App.renderMeetings = function () {
    ...
    if (el) el.innerHTML = `<div class="empty">${tr('会议模块暂不可用')}</div>`;
  };

  // 打开会议记录编辑/上传弹窗（index.html 上传会议记录按钮调用）
  App.openMeetingEditor = function (id) {
    if (!this.canEdit('meetings')) { this.toast(tr('只读模式，无法上传')); return; }
    ...
    const html =
      ...
      `<div class="input-group"><label>${tr('主题')}</label><input type="text" id="mt-title" value="${this.esc(title)}" placeholder="${tr('会议主题')}"></div>` +
      `<div class="input-group"><label>${tr('日期')}</label><input type="date" id="mt-date" value="${this.esc(date)}"></div>` +
      `<div class="input-group"><label>${tr('纪要')}</label><textarea id="mt-notes" placeholder="${tr('会议内容 / 决议')}">${this.esc(notes)}</textarea></div>` +
      '<button class="btn btn-primary" onclick="App.saveMeeting(' + (id || 'null') + ')">' + tr('保存') + '</button>' +
  ...
  App.saveMeeting = async function (id) {
    ...
    if (!title) { this.toast(tr('主题不能为空')); return; }
