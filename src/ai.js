...
export function initAIAssistant() {
  ...
  urlSave.addEventListener('click', () => {
    ...
    if (!v) { alert('请输入 AI 后端地址'); return; }
    localStorage.setItem(STORE_URL, v);
    settingsBox.style.display = 'none';
    addMsg('bot', '✅ 已设置 AI 后端，可以开始提问了。');
  });
  ...
  async function send() {
    ...
    if (!url) {
      ...
      urlInput.focus();
      addMsg('bot', '⚠️ 请先在设置中填入 AI 后端地址（部署后端后获得的 CloudStudio 链接）。');
      return;
    }
    inputEl.value = '';
    addMsg('user', text);
    history.push({ role: 'user', content: text });
    const typing = addMsg('bot', '小P 正在思考…');
    try {
      ...
      typing.textContent = '❌ 调用失败：' + e.message + '\n请确认后端地址正确且服务已在运行（CloudStudio 端口预览已开启）。';
    }
  }
  ...
  addMsg('bot', '您好，我是 PMApp AI 助理「小P」。可以用中文或任何语言问我系统里的资料，例如：\n• 越南工厂本周有哪些延误的生产子计划？\n• 列出所有进行中的项目及其交期\n• 最近有哪些严重质量问题？');
}
